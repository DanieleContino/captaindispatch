...
function FleetMonitor({ onBack }) {
  ...
  const loadFleet = useCallback(async (d) => {
    ...
    const [vRes, tRes, lRes] = await Promise.all([
      supabase.from('vehicles').select('uuid,display_id,driver_name,sign_code,capacity,vehicle_type').eq('production_id', PRODUCTION_ID).eq('active', true).order('display_id'),
...
function WrapTripContent() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      supabase.from('locations').select('uuid,display_id,name,is_hub').eq('production_id', PRODUCTION_ID).order('is_hub', { ascending: true }).order('name'),
      supabase.from('vehicles').select('uuid,display_id,driver_name,sign_code,capacity,vehicle_type').eq('production_id', PRODUCTION_ID).eq('active', true).order('display_id'),
      supabase.from('crew').select('uuid,display_id,full_name,department,hotel_id').eq('production_id', PRODUCTION_ID).eq('hotel_status', 'CONFIRMED').order('department').order('full_name'),
    ]).then(([lR, vR, cR]) => {
