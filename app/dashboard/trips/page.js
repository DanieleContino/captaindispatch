...
function WaypointReviewModal({ open, group, locations, productionId, onClose, onSaved }) {
  ...
  const [error,      setError]      = useState(null)
  const [saving,     setSaving]     = useState(false)
  const [legs,       setLegs]       = useState([])        // legs in current display order
  const [optimized,  setOptimized]  = useState(null)      // optimized_order from API
  ...
  useEffect(() => {
    ...
    fetch('/api/routes/optimize-waypoints', {
      ...
      .then(data => {
        ...
        // Build ordered legs array from optimized_order
  ...
  async function handleConfirm() {
    ...
    try {
      // Update leg_order for each leg according to new display order
      ...
      // Recompute chain timing with new order
      await fetch('/api/routes/compute-chain', {
        method:  'POST',
        headers: { 'Content-Type': 'application/json' },
...
function ReplicaDayModal({ open, onClose, sourceDate, targetDate, locations, onDone }) {
  ...
  // Carica trip di sourceDate quando la modale si apre
  useEffect(() => {
    ...
    supabase.from('trips').select('*')
      ...
      .then(({ data }) => {
        const rows = data || []
        setPrevTrips(rows)
        // Seleziona tutto per default
        const keys = new Set()
        for (const t of rows) keys.add(baseTripId(t.trip_id) + '::' + (t.vehicle_id || '__none__'))
        setSelected(keys)
        setLoading(false)
      })
  ...
  const allKeys      = grouped.map(g => baseTripId(g[0].trip_id) + '::' + (g[0].vehicle_id || '__none__'))
  const allSelected  = allKeys.length > 0 && allKeys.every(k => selected.has(k))
  const selectedCount = grouped.filter(g => selected.has(baseTripId(g[0].trip_id) + '::' + (g[0].vehicle_id || '__none__'))).length
  ...
  async function handleCopy() {
    ...
    try {
      const groupsToCopy = grouped.filter(g =>
        selected.has(baseTripId(g[0].trip_id) + '::' + (g[0].vehicle_id || '__none__'))
      )

      // Trova il prossimo numero trip_id disponibile (globalmente, non solo per la data)
      ...
      for (const group of groupsToCopy) {
        ...
        for (let i = 0; i < group.length; i++) {
          ...
          // Ricalcola start_dt / end_dt sulla nuova data
      ...
      {!isMobile && <PageHeader
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
          ...
          <button onClick={() => { setNewTripOpen(true); setEditTripRow(null) }}
            style={{ background: '#2563eb', color: 'white', border: 'none', borderRadius: '8px', padding: '7px 16px', fontSize: '13px', fontWeight: '800', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px', boxShadow: '0 2px 8px rgba(37,99,235,0.3)', whiteSpace: 'nowrap' }}>
            + New Trip
          </button>
