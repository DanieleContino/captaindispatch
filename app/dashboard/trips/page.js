      ...
      <div style={{ background: '#0f2340', padding: '0 24px', height: '52px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 30 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
          ...
          <nav style={{ display: 'flex', gap: '2px' }}>
            {NAV.map(({ l, p }) => (
              <a key={p} href={p} style={{ padding: '5px 12px', borderRadius: '7px', fontSize: '13px', fontWeight: '600', color: p === '/dashboard/trips' ? 'white' : '#94a3b8', background: p === '/dashboard/trips' ? '#1e3a5f' : 'transparent', textDecoration: 'none', whiteSpace: 'nowrap' }}>{l}</a>
            ))}
          </nav>
        </div>
