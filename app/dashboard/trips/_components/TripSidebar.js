...
import TripSidebarMulti from './TripSidebarMulti'
...
function TripSidebar({ open, onClose, defaultDate, locations, vehicles, serviceTypes, onSaved, assignCtx, trips, onLocationCreated, currentUser }) {
    ...
    try {
      for (let i = 0; i < allLegs.length; i++) {
        ...
        const row = {
          ...
          pickup_min:      legForm.pickup_time ? timeStrToMin(legForm.pickup_time) : (legComp?.pickupMin ?? null),
          start_dt:        legForm.pickup_time ? (() => { const pm = timeStrToMin(legForm.pickup_time); const [y,mo,dd] = legForm.date.split('-').map(Number); return new Date(y,mo-1,dd,Math.floor(pm/60),pm%60,0,0).toISOString() })() : (legComp?.startDt ?? null),
  ...
  return (
    <>
      ...
      {(tripMode === 'MULTI-PICK' || tripMode === 'MULTI-DROP' || tripMode === 'MISTO') && (
        <TripSidebarMulti
          open={open && tripMode !== 'SINGLE'}
          onClose={onClose}
          onSaved={onSaved}
          locations={locations}
          vehicles={vehicles}
