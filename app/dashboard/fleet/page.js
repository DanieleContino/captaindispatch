...
function vehicleStatus(groups, now) {
  ...
  // FREE / SOON: ci sono trip PLANNED
  if (plannedGroups.length > 0) {
    const next = plannedGroups.find(g => g.minStart && g.minStart > now) || plannedGroups[0]
    const diffMin = next?.minStart ? (next.minStart - now) / 60000 : Infinity
    const status = (diffMin >= 0 && diffMin <= 30) ? 'SOON' : 'FREE'
    return { status, estimated: false, current: null, next, last: null }
  }
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f1f5f9' }}>
      ...
      <div style={{ maxWidth: '1280px', margin: '0 auto', padding: isMobile ? '12px' : '24px', overflowX: 'hidden' }}>
        ...
        {/* ── Mappa Google Maps + Lista Live (solo desktop) ── */}
        {!isMobile && mapOpen && (
                  ...
                  {sessions.map((s, i) => {
                    ...
                    return (
                      <div key={s.id || i} style={{
                        ...
                        <span style={{ fontSize: '10px', fontWeight: '800', padding: '2px 8px', borderRadius: '999px', whiteSpace: 'nowrap', background: st.badgeBg, color: st.badge, border: `1px solid ${st.border}` }}>
                          {st.label}
                        </span>
