...
<div style={{ display: 'grid', gridTemplateColumns: '1fr 110px', gap: '8px', marginBottom: '12px' }}>
              <div style={{ position: 'relative' }}>
                ...
                <input ref={fieldRefs.from_location} value={form.from_location} onChange={e => { set('from_location', e.target.value); setFromOpen(true) }} onFocus={() => setFromOpen(true)} onBlur={() => setTimeout(() => setFromOpen(false), 150)} style={inp} placeholder="Rome FCO" autoComplete="off" />
                {fromOpen && form.from_location.length > 0 && hubs.filter(h => h.name.toLowerCase().includes(form.from_location.toLowerCase()) || (h.display_id || '').toLowerCase().includes(form.from_location.toLowerCase())).length > 0 && (
                  <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, zIndex: 200, background: 'white', border: '1px solid #e2e8f0', borderRadius: '8px', marginTop: '2px', overflow: 'hidden', boxShadow: '0 4px 12px rgba(0,0,0,0.1)' }}>
                    {hubs.filter(h => h.name.toLowerCase().includes(form.from_location.toLowerCase()) || (h.display_id || '').toLowerCase().includes(form.from_location.toLowerCase())).map(h => (
                      <div key={h.uuid} onClick={() => { set('from_location', h.name); setFromOpen(false) }}
...
<div style={{ display: 'grid', gridTemplateColumns: '1fr 110px', gap: '8px', marginBottom: '12px' }}>
            ...
            <div style={{ marginBottom: '12px', padding: '10px 12px', background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: '8px' }}>
              ...
              {/* Dropdown manuale — solo se link automatico mancante */}
              {form.crew_id && crewStays.length > 0 && !form.linked_stay_id && (
              ...
              {form.crew_id && crewStays.length > 1 && !!form.linked_stay_id && (
                <div style={{ marginBottom: '8px' }}>
                  {!showStayOverride ? (
                    <button type="button" onClick={() => setShowStayOverride(true)}
                      style={{ fontSize: '10px', color: '#64748b', background: 'none', border: 'none', cursor: 'pointer', padding: '0', textDecoration: 'underline' }}>
                      ✎ Override linked stay
                    </button>
                  ) : (
                    <>
                      <div style={{ fontSize: '10px', color: '#92400e', background: '#fff7ed', border: '1px solid #fde68a', borderRadius: '6px', padding: '5px 8px', marginBottom: '6px', lineHeight: 1.4 }}>
                        ℹ Multiple stays found. The link was set automatically — change only if incorrect.
                      </div>
                      <label style={lbl}>Link stay</label>
                      <select
                        value={form.linked_stay_id || ''}
                      ...
                      <button type="button" onClick={() => setShowStayOverride(false)}
                        style={{ fontSize: '10px', color: '#94a3b8', background: 'none', border: 'none', cursor: 'pointer', padding: '4px 0 0', textDecoration: 'underline' }}>
                        Cancel override
                      </button>
                    </>
                  )}
...
// ─── NoMovementsBanner ───────────────────────────────────────
function NoMovementsBanner({ crew, onAddMovement, onDismiss }) {
    ...
    <div style={{ background: '#fef2f2', border: '1px solid #fecaca', borderRadius: '10px', marginBottom: '16px', overflow: 'hidden' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 16px', cursor: 'pointer' }}
        ...
        <span style={{ fontSize: '13px', fontWeight: '800', color: '#dc2626', flex: 1 }}>
          ⚠ {crew.length} crew member{crew.length !== 1 ? 's' : ''} have accommodation but no travel movements
        </span>
        <span style={{ fontSize: '11px', fontWeight: '700', color: '#dc2626', whiteSpace: 'nowrap' }}>
          {expanded ? '▲ Hide' : '▼ Show'}
        </span>
        <button onClick={e => { e.stopPropagation(); onDismiss() }}
          style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#94a3b8', fontSize: '16px', lineHeight: 1, padding: '2px 4px', flexShrink: 0 }}>✕</button>
      </div>
      {expanded && (
        <div style={{ borderTop: '1px solid #fecaca', maxHeight: '320px', overflowY: 'auto' }}>
          {crew.map((c, i) => (
            <div key={c.uuid} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '8px 16px', borderBottom: i < crew.length - 1 ? '1px solid #fef2f2' : 'none', background: 'white' }}>
              ...
              {c.role && <span style={{ fontSize: '11px', color: '#dc2626', flexShrink: 0 }}>{c.role}</span>}
              <button onClick={() => onAddMovement({ crew_id: c.uuid, crew_full_name: c.full_name })}
                style={{ padding: '3px 10px', borderRadius: '6px', border: 'none', background: '#dc2626', color: 'white', fontSize: '11px', fontWeight: '700', cursor: 'pointer', flexShrink: 0, whiteSpace: 'nowrap' }}>
                + Add Movement
              </button>
            </div>
          ))}
...
function TravelPendingCrewBanner({ crew, onAddMovement, onDismiss }) {
  const [expanded, setExpanded] = React.useState(false)
  return (
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f1f5f9' }}>
      ...
      <div style={{ padding: isMobile ? '12px' : '16px 24px' }}>
        ...
        {noMovementsCrew.length > 0 && (
          <NoMovementsBanner crew={noMovementsCrew} onAddMovement={m => openNew({ crew_id: m.crew_id, crew_full_name: m.crew_full_name })} onDismiss={() => setNoMovementsCrew([])} />
        )}
        {pendingCrew.length > 0 && (
          <TravelPendingCrewBanner crew={pendingCrew} onAddMovement={m => openNew({ crew_id: m.crew_id, crew_full_name: m.crew_full_name })} onDismiss={() => setPendingCrew([])} />
        )}
