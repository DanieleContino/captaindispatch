...
// Inizializzazione VAPID lazy — evita crash al module load se le env vars
// non sono ancora disponibili (es. durante il build su Vercel senza env vars)
...
function ensureVapidInit() {
  ...
  if (!email || !pubKey || !privKey) {
    console.warn('[webpush] VAPID env vars mancanti — push disabilitato')
    return
  }
...
async function _sendToSubscriptions(supabase, subscriptions, payload) {
  ...
  if (!_vapidInitialized) {
    console.warn('[webpush] Push saltato — VAPID non inizializzato')
    return { sent: 0, errors: 0 }
  }
