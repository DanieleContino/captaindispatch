'use client'
/**
 * TravelColumnsEditorSidebar.js
 * Session S55 · 12 May 2026
 *
 * Sidebar editor for /dashboard/travel column configuration.
 * Allows adding, removing, reordering and resizing columns.
 * Persists configuration to Supabase table `travel_columns`.
 *
 * Architecture mirrors lib/ColumnsEditorSidebar.js (used by Lists).
 */

import { useEffect, useRef, useState } from 'react'
import { supabase } from './supabase'
import { getProductionId } from './production'
import { TRAVEL_COLUMNS_CATALOG, TRAVEL_DEFAULT_PRESET } from './travelColumnsCatalog'
...
// ─── Main sidebar ──────────────────────────────────────────────
export function TravelColumnsEditorSidebar({ open, onClose, onChanged }) {
  ...
  function startEdit(c) {
    ...
    setError(null)
    // Auto-scroll to the edit form so the user can see it
    setTimeout(() => {
  ...
  function pickField(key) {
    const def = TRAVEL_COLUMNS_CATALOG[key]
    if (!def) return
    setForm(f => ({
      ...f,
      source_field: key,
  ...
  async function handleSave() {
    ...
    if (!TRAVEL_COLUMNS_CATALOG[form.source_field]) {
  ...
  async function handleResetToDefaultPreset() {
    if (!PRODUCTION_ID) return
    if (columns.length > 0) {
      const ok = confirm('This will REPLACE all current columns with the 13-column default preset. Continue?')
      if (!ok) return
    }
    setSaving(true); setError(null)
    try {
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{ padding: '14px 18px', background: '#0f2340', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
          <div>
            <div style={{ fontSize: '10px', color: '#94a3b8', letterSpacing: '0.06em', textTransform: 'uppercase' }}>✈️ Travel</div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: '14px' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                    {columns.map(c => (
                      <SortableColumnRow
                        key={c.id}
                        column={c}
                        def={TRAVEL_COLUMNS_CATALOG[c.source_field]}
          ...
          {/* Add / Edit form */}
          <div ref={formRef} style={{ border: '1px dashed #cbd5e1', borderRadius: '8px', padding: '10px 12px', background: editingId ? '#fffbeb' : '#f8fafc', borderColor: editingId ? '#fbbf24' : '#cbd5e1' }}>
            ...
            <div style={{ marginBottom: '8px' }}>
              ...
              <select
                ...
                {Object.entries(TRAVEL_COLUMNS_CATALOG).map(([key, def]) => (
