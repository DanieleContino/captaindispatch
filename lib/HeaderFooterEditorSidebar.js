...
import { BlockConfigForm } from './BlockConfigForms'
...
// ─── Sortable block row ──────────────────────────────────────
function SortableBlockRow({ block, isExpanded, onToggle, onDelete, onPatch }) {
  const {
    ...
    <div ref={setNodeRef} style={style}>
      ...
      {isExpanded && (
        <div style={{
          ...
          <BlockConfigForm block={block} onPatch={onPatch} />
        </div>
      )}
    </div>
  )
...
function ZoneSection({ zone, blocks, expandedId, setExpandedId, onReorder, onAdd, onDelete, onPatch }) {
