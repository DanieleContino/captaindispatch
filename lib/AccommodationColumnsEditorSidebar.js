...
import {
  ACCOMMODATION_COLUMNS_CATALOG,
  ACCOMMODATION_DEFAULT_PRESET,
  CALENDAR_COST_DEFAULT_PRESET,
} from './accommodationColumnsCatalog'
...
const WIDTH_OPTIONS = [
  { value: '38px',  label: '38px (icon only)' },
  { value: '44px',  label: '44px (badge)' },
  { value: '52px',  label: '52px (very narrow)' },
  { value: '56px',  label: '56px (time/nights)' },
  { value: '70px',  label: '70px (narrow)' },
  { value: '76px',  label: '76px (narrow)' },
  { value: '80px',  label: '80px (small)' },
  { value: '88px',  label: '88px (medium-small)' },
  { value: '90px',  label: '90px (medium-small)' },
  { value: '100px', label: '100px (medium)' },
  { value: '110px', label: '110px (medium)' },
  { value: '120px', label: '120px (medium-wide)' },
  { value: '130px', label: '130px (wide)' },
...
function SortableColumnRow({ column, isEditing, def, onStartEdit, onCancelEdit, onDelete }) {
  ...
  const style = {
    ...
    gap: '6px',
    padding: '7px 10px',
    background: isEditing ? '#f0fdf4' : '#f8fafc',
    border: '1px solid ' + (isEditing ? '#86efac' : '#e2e8f0'),
    borderRadius: '7px',
    touchAction: 'none',
  }
  return (
    <div ref={setNodeRef} style={style}>
      ...
      <button
        ...
        style={{ background: isEditing ? '#15803d' : 'white', border: '1px solid #e2e8f0', color: isEditing ? 'white' : '#64748b', borderRadius: '5px', padding: '3px 8px', cursor: 'pointer', fontSize: '11px', fontWeight: '700', flexShrink: 0 }}>
...
export function AccommodationColumnsEditorSidebar({ open, onClose, onChanged, viewType = 'list' }) {
  const PRODUCTION_ID = getProductionId()
  const [columns,   setColumns]   = useState([])
  const [loading,   setLoading]   = useState(false)
  const [error,     setError]     = useState(null)
  const [editingId, setEditingId] = useState(null)
  const [form,      setForm]      = useState({ header_label: '', source_field: '', width: '110px' })
  const [saving,    setSaving]    = useState(false)
  const formRef = useRef(null)

  const sensors = useSensors(useSensor(PointerSensor, { activationConstraint: { distance: 8 } }))

  const isCalendar = viewType === 'calendar_cost'
  const defaultPreset = isCalendar ? CALENDAR_COST_DEFAULT_PRESET : ACCOMMODATION_DEFAULT_PRESET
  const catalogEntries = Object.entries(ACCOMMODATION_COLUMNS_CATALOG).filter(([, def]) => def.view_type === viewType)
  ...
  useEffect(() => { if (open) { cancelEdit(); loadColumns() } }, [open, viewType])

  function startEdit(c) {
    setEditingId(c.id)
    setForm({ header_label: c.header_label, source_field: c.source_field, width: c.width || '110px' })
    setError(null)
  ...
  function pickField(key) {
    const def = ACCOMMODATION_COLUMNS_CATALOG[key]
    if (!def) return
    setForm(f => ({
      ...f,
      source_field: key,
      header_label: f.header_label || def.label,
      width:        f.width !== '110px' ? f.width : (def.defaultWidth || '110px'),
    }))
  }
  ...
  async function handleSave() {
    ...
    if (!ACCOMMODATION_COLUMNS_CATALOG[form.source_field]) {
  ...
  async function handleDelete(c) {
    ...
    setError(null)
    const { error } = await supabase.from('accommodation_columns').delete().eq('id', c.id)
    if (error) { setError(error.message); return }
    await loadColumns()
    onChanged?.()
  }

  async function handleResetToDefaultPreset() {
    if (!PRODUCTION_ID) return
    if (columns.length > 0) {
      const ok = confirm('This will REPLACE all current ' + (isCalendar ? 'calendar cost' : 'list') + ' columns with the default preset. Continue?')
      if (!ok) return
    }
    setSaving(true); setError(null)
    try {
      ...
      const rows = defaultPreset.map(p => ({ ...p, production_id: PRODUCTION_ID }))
      const { error: insErr } = await supabase.from('accommodation_columns').insert(rows)
  ...
  const headerBg = isCalendar ? '#2563eb' : '#15803d'
  const headerLabel = isCalendar ? 'Calendar Costs' : 'List View'
  ...
  return (
    <>
      ...
      <div style={{
        ...
        <div style={{ padding: '14px 18px', background: headerBg, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
          <div>
            <div style={{ fontSize: '10px', color: 'rgba(255,255,255,0.7)', letterSpacing: '0.06em', textTransform: 'uppercase' }}>🏨 Accommodation — {headerLabel}</div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: '14px' }}>
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' }}>
              ...
              <button
                ...
                disabled={saving}
                style={{ fontSize: '10px', padding: '3px 8px', borderRadius: '5px', border: `1px solid ${headerBg}`, background: 'white', color: headerBg, cursor: saving ? 'default' : 'pointer', fontWeight: '700' }}>
                Reset to Default
              </button>
            </div>
            ...
            {loading ? (
              ...
              <div style={{ padding: '14px', textAlign: 'center', color: '#cbd5e1', fontSize: '12px', fontStyle: 'italic', border: '1px dashed #e2e8f0', borderRadius: '7px' }}>
                No columns yet. Add one below or click &quot;Reset to Default&quot;.
              </div>
            ) : (
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                    {columns.map(c => (
                      <SortableColumnRow
                        key={c.id}
                        column={c}
                        def={ACCOMMODATION_COLUMNS_CATALOG[c.source_field]}
          ...
          <div ref={formRef} style={{ border: '1px dashed #cbd5e1', borderRadius: '8px', padding: '10px 12px', background: editingId ? '#f0fdf4' : '#f8fafc', borderColor: editingId ? '#86efac' : '#cbd5e1' }}>
            ...
            <div style={{ marginBottom: '8px' }}>
              ...
              <input
                ...
                placeholder="e.g. Check-in, Name, Room…"
                style={inp} />
            </div>
            ...
            <div style={{ display: 'flex', gap: '6px' }}>
              ...
              <button
                ...
                style={{
                  ...
                  background: (saving || !form.header_label.trim() || !form.source_field) ? '#cbd5e1' : headerBg,
        ...
        {error && (
          <div style={{ margin: '0 16px 12px', padding: '8px 12px', background: '#fef2f2', border: '1px solid #fecaca', borderRadius: '7px', color: '#dc2626', fontSize: '12px' }}>
            Error: {error}
          </div>
        )}
