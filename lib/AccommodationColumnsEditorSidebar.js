'use client'
/**
 * AccommodationColumnsEditorSidebar.js
 *
 * Sidebar editor for /dashboard/accommodation column configuration.
 * Allows adding, removing, reordering and resizing columns.
 * Persists configuration to Supabase table `accommodation_columns`.
 *
 * Mirrors lib/TravelColumnsEditorSidebar.js exactly.
 */

import { useEffect, useRef, useState } from 'react'
import { supabase } from './supabase'
import { getProductionId } from './production'
import { ACCOMMODATION_COLUMNS_CATALOG, ACCOMMODATION_DEFAULT_PRESET } from './accommodationColumnsCatalog'
...
export function AccommodationColumnsEditorSidebar({ open, onClose, onChanged }) {
  ...
  async function handleResetToDefaultPreset() {
    if (!PRODUCTION_ID) return
    if (columns.length > 0) {
      const ok = confirm('This will REPLACE all current columns with the default preset. Continue?')
      if (!ok) return
    }
    setSaving(true); setError(null)
    try {
  ...
  return (
    <>
      ...
      <div style={{
        ...
        <div style={{ padding: '14px 18px', background: '#15803d', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
          <div>
            <div style={{ fontSize: '10px', color: 'rgba(255,255,255,0.7)', letterSpacing: '0.06em', textTransform: 'uppercase' }}>🏨 Accommodation</div>
          ...
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' }}>
              ...
              <button
                ...
                disabled={saving}
                style={{ fontSize: '10px', padding: '3px 8px', borderRadius: '5px', border: '1px solid #15803d', background: 'white', color: '#15803d', cursor: saving ? 'default' : 'pointer', fontWeight: '700' }}>
                Reset to Default
              </button>
            </div>
          ...
          <div ref={formRef} style={{ border: '1px dashed #cbd5e1', borderRadius: '8px', padding: '10px 12px', background: editingId ? '#f0fdf4' : '#f8fafc', borderColor: editingId ? '#86efac' : '#cbd5e1' }}>
            ...
            <div style={{ marginBottom: '8px' }}>
              ...
              <select
                ...
                {Object.entries(ACCOMMODATION_COLUMNS_CATALOG).map(([key, def]) => (
            ...
            <div style={{ display: 'flex', gap: '6px' }}>
              ...
              <button
                ...
                style={{
                  ...
                  background: (saving || !form.header_label.trim() || !form.source_field) ? '#cbd5e1' : '#15803d',
