...
export const COLUMNS_CATALOG = {
  ...
  vehicle_id: {
    ...
    defaultWidth: '100px',
    render: (group, ctx) => (
      <span style={{ fontWeight: 700 }}>{ctx?.vehicleMap?.[group.vehicle_id] || group.vehicle_id || '—'}</span>
    ),
  },
  vehicle_capacity: {
  ...
  pickup_min_hhmm: {
    ...
    defaultWidth: '70px',
    render: (group) => {
      const row = group._currentRow || group.rows?.[0] || group
      return (
        <span style={{ fontWeight: 700, fontVariantNumeric: 'tabular-nums' }}>
          {minToHHMM(row.pickup_min ?? row.call_min ?? group.pickup_min ?? group.call_min)}
        </span>
      )
    },
  },
  ...
  pickup_call_2lines: {
    ...
    render: (group) => {
      const row = group._currentRow || group.rows?.[0] || group
      const pickup = minToHHMM(row.pickup_min ?? row.call_min ?? group.pickup_min ?? group.call_min)
  ...
  passengers_lastname_role: {
    ...
    category: 'Passengers',
    defaultWidth: '1fr',
    render: (group, ctx, legIndex) => {
      const isMultiLeg = group.trip_group_id && group.rows.length > 1
      const row = group.rows[legIndex ?? 0]
      const currentPax = (ctx?.paxByTripRow && ctx.paxByTripRow[row?.id]) || []
      const prevPax = isMultiLeg && legIndex > 0
        ? (ctx?.paxByTripRow && ctx.paxByTripRow[group.rows[legIndex - 1]?.id]) || []
        : []
      const prevUuids = new Set(prevPax.map(c => c.uuid))
      const boarding = isMultiLeg
        ? currentPax.filter(c => !prevUuids.has(c.uuid))
        : currentPax

      let enriched = boarding.map(c => ({ name: c.full_name, role: c.role || c.department || null }))
      if (enriched.length === 0 && !isMultiLeg && row?.passenger_list) {
        enriched = row.passenger_list.split(',').map(s => s.trim()).filter(Boolean).map(n => ({ name: n, role: null }))
      }
      if (enriched.length === 0) {
        return <span style={{ color: '#cbd5e1', fontStyle: 'italic', fontSize: 11 }}>{isMultiLeg && legIndex > 0 ? 'no boarding' : 'no pax'}</span>
      }
      return (
        <div>
          {enriched.map((p, i) => (
  ...
  pickup_name: {
    ...
    defaultWidth: '130px',
    render: (group, ctx) => {
      const row = group._currentRow || group.rows?.[0]
      const loc = ctx?.locsMap?.[row?.pickup_id]
      const name = (typeof loc === 'object' ? loc?.name : loc) || row?.pickup_id || '—'
  ...
  pickup_full_with_map_compact: {
    label: 'Pickup: name + address + maps link (compact print)',
    category: 'Pickup',
    defaultWidth: '180px',
    render: (group, ctx) => {
      const row = group._currentRow || group.rows?.[0]
      ...
      return (
        <div>
          ...
          {url && (
            <a href={url} target="_blank" rel="noopener noreferrer"
              style={{ fontSize: '11px', color: '#1d4ed8', textDecoration: 'none', marginTop: 2, display: 'inline-block' }}>
              🗺 Map
            </a>
          )}
        </div>
  ...
  pickup_full_with_map_url: {
    label: 'Pickup: name + address + maps link (URL printed)',
    category: 'Pickup',
    defaultWidth: '200px',
    render: (group, ctx) => {
  ...
  dropoff_name: {
    ...
    defaultWidth: '130px',
    render: (group, ctx) => {
      const last = group._currentRow || group.rows?.[group.rows.length - 1]
  ...
  dropoff_name_flight: {
    ...
    defaultWidth: '160px',
    render: (group, ctx, legIndex) => {
      const currentRow = group._currentRow || group.rows?.[legIndex ?? 0]
      const rows = group.rows || []
      const isMultiPick = group.rows?.length > 1 && rows.some(r => ['MULTI-PICK','Multi-Pick','MULTI-PKP'].includes(r?.service_type))
      const isMultiDrop = group.rows?.length > 1 && rows.some(r => ['MULTI-DROP','Multi-Drop'].includes(r?.service_type))
      const li = legIndex ?? 0
      const isLastLeg = li === rows.length - 1
      let locId
      if (isMultiPick && !isLastLeg) {
        // TO = pickup del leg successivo
        locId = rows[li + 1]?.pickup_id
      } else if (isMultiDrop && li > 0) {
        // per MULTI-DROP il TO è sempre il dropoff del leg corrente
        locId = currentRow?.dropoff_id
      } else {
        locId = currentRow?.dropoff_id
      }

      const loc = ctx?.locsMap?.[locId]
      const name = (typeof loc === 'object' ? loc?.name : loc) || locId || '—'
      const flight = group.flight_no || currentRow?.flight_no
  ...
  dropoff_full_with_map_compact: {
    label: 'Dropoff: name + address + flight + maps link (compact print)',
    category: 'Dropoff',
    defaultWidth: '180px',
    render: (group, ctx, legIndex) => {
      const rows = group.rows || []
      const li = legIndex ?? 0
      const isLastLeg = li === rows.length - 1
      const currentRow = group._currentRow || rows[li]
      const isMultiPick = rows.length > 1 && rows.some(r => ['MULTI-PICK','Multi-Pick'].includes(r?.service_type))
      let locId
      if (isMultiPick && !isLastLeg) {
        locId = rows[li + 1]?.pickup_id
      } else {
  ...
  dropoff_full_with_map_url: {
    label: 'Dropoff: name + address + flight + maps link (URL printed)',
    category: 'Dropoff',
    defaultWidth: '200px',
    render: (group, ctx) => {
      const last = group._currentRow || group.rows?.[group.rows.length - 1]
...
export const CAPTAIN_PRESET = [
  { source_field: 'sign_code',                   header_label: 'Sign',       width: '90px',  display_order: 10 },
