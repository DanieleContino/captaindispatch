...
export const COLUMNS_CATALOG = {
  ...
  dropoff_name_flight: {
    ...
    render: (group, ctx, legIndex) => {
      ...
      const isMultiPick = group.rows?.length > 1 && ['MULTI-PICK','Multi-Pick','MULTI-PKP'].includes(rows[0]?.service_type)
      const isMultiDrop = group.rows?.length > 1 && ['MULTI-DROP','Multi-Drop'].includes(rows[0]?.service_type)
      const li = legIndex ?? 0
