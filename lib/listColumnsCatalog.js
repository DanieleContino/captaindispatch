...
export const COLUMNS_CATALOG = {
  ...
  dropoff_name_flight: {
    ...
    render: (group, ctx, legIndex) => {
      ...
      const isMultiPick = group.rows?.length > 1 && rows[0]?.service_type === 'MULTI-PICK' || rows[0]?.service_type === 'Multi-Pick' || rows[0]?.service_type === 'MULTI-PKP'
      const isMultiDrop = group.rows?.length > 1 && rows[0]?.service_type === 'MULTI-DROP' || rows[0]?.service_type === 'Multi-Drop'
      const li = legIndex ?? 0
