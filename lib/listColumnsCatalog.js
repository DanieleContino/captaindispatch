...
export const COLUMNS_CATALOG = {
  ...
  driver_phone: {
    ...
    defaultWidth: '120px',
    render: (group) => (
      <span style={{ fontSize: '11px' }}>{group.rows?.[0]?.driver_phone || '—'}</span>
    ),
  },
  driver_name_phone_2lines: {
    ...
    defaultWidth: '130px',
    render: (group) => {
      const phone = group.rows?.[0]?.driver_phone
      return (
        <div>
    ...
    render: (group, ctx) => {
      ...
      return (
        <div>
          ...
          <div style={{ fontSize: '8px', color: '#94a3b8', marginTop: 2, wordBreak: 'break-all', lineHeight: 1.3 }}>
            {url}
          </div>
        </div>
      )
