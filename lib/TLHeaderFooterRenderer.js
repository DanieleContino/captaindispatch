...
export function TLHeaderFooterRenderer({ productionId, zone, currentDate, shootStart, shootEnd, reloadKey, onOpenEditor }) {
  ...
  // Pull logoUrl from the logo_image block config (stored there after upload)
  const logoUrl = blocks.find(b => b.block_type === 'logo_image')?.config?.logo_url || null
  ...
  const ctx = {
    ...
    teamContacts: [],     // TODO next task: pull from crew table + overrides
    logoUrl,
    pageNumber: null,
    totalPages: null,
  }
