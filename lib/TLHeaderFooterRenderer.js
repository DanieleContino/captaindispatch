...
export function TLHeaderFooterRenderer({ productionId, zone, currentDate, shootStart, shootEnd, reloadKey, onOpenEditor }) {
  ...
  const allZoneBlocks = (template.blocks || [])
  ...
  // Group: top-level blocks + their children
  const topLevelBlocks = allZoneBlocks
    .filter(b => !b.parent_block_id)
    .sort((a, b) => (a.display_order || 0) - (b.display_order || 0))

  const childrenByParent = {}
  for (const b of allZoneBlocks) {
    if (b.parent_block_id) {
      if (!childrenByParent[b.parent_block_id]) childrenByParent[b.parent_block_id] = []
      childrenByParent[b.parent_block_id].push(b)
    }
  }
  // Sort children inside each parent
  for (const k of Object.keys(childrenByParent)) {
    childrenByParent[k].sort((a, b) => (a.display_order || 0) - (b.display_order || 0))
  }

  // For grid layout: top-level blocks define the columns
      ...
      {blocks.map(b => {
        ...
        return (
          <div
            ...
            style={{
              ...(useFlex ? {} : { minWidth: 0 }),
              ...(hasChildren ? { display: 'flex', flexDirection: 'column', gap: 4 } : {}),
            }}
          >
            <div>{renderBlock(b.block_type, b.config, ctx)}</div>
            {children.map(c => (
              <div key={c.id}>{renderBlock(c.block_type, c.config, ctx)}</div>
            ))}
          </div>
        )
      })}
