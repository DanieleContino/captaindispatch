...
import { loadActiveTemplate } from './tlTemplatesDb'
 ...
 */
export function TLHeaderFooterRenderer({ productionId, zone, currentDate, reloadKey, onOpenEditor }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setLoading(true)
      try {
        const active = await loadActiveTemplate(productionId)
        if (!cancelled) setData(active)
      } catch (e) {
  ...
  const ctx = {
    ...
    shootStart: null,     // TODO Task 9: pull from productions table
    shootEnd: null,
    teamContacts: [],     // TODO Task 9: pull from crew table + overrides
    logoUrl: null,        // TODO Task 8: pull from Supabase Storage
    pageNumber: null,
    totalPages: null,
  }
