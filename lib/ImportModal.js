...
function isUnrecognized(row, mode) {
  ...
  if (mode === 'accommodation') return !row.first_name && !row.last_name
  return false
}
...
export function ImportModal({ open, mode: initialMode, productionId, locations = [], onClose, onImported, initialPhase, initialRows, initialNewHotels, initialDetectedMode, initialSelMode }) {
  ...
  async function handleDriveSync(fileId) {
    ...
    try {
      ...
      const s = d.summary || {}
      setDriveMsg(
        `✅ Sync done — ${s.synced || 0} synced, ${s.skipped || 0} unchanged, ${s.failed || 0} failed`
      )
      await loadDriveFiles()
          ...
          {phase === 'idle' && (
            <div style={{ padding: '24px' }}>
              ...
              <div style={{ marginTop: '24px', borderTop: '1px solid #e2e8f0', paddingTop: '20px' }}>
                ...
                {/* Drive messages */}
                {driveError && (
                  <div style={{ marginBottom: '10px', padding: '8px 12px', background: '#fef2f2', border: '1px solid #fecaca', borderRadius: '8px', fontSize: '12px', color: '#dc2626', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '8px' }}>
                    <span>❌ {driveError}</span>
                    <button onClick={() => setDriveError(null)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#dc2626', fontSize: '14px', padding: 0, lineHeight: 1 }}>✕</button>
                  </div>
                )}
                {driveMsg && (
                ...
                {driveLoading ? (
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                    {driveFiles.map(f => {
                      const MODE_LABELS = { hal: '🔴 HAL', crew: '👥 Crew', accommodation: '🏨 Accomm.', fleet: '🚗 Fleet' }
