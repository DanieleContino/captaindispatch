...
// ── Fleet preview table ─────────────────────────────────────
function FleetTable({ rows, onToggleAction, onEdit, onSelectAll }) {
  const dupRows = rows.filter(r => r.existingId)
  const allUpdate = dupRows.length > 0 && dupRows.every(r => r.action === 'update')
  return (
        ...
        <thead>
          <tr style={{ background: '#f8fafc' }}>
            ...
            <th style={{ ...TH, textAlign: 'center' }}>
              ...
              {dupRows.length > 0 && (
                <button
                  onClick={() => onSelectAll(allUpdate ? 'skip' : 'update')}
                  style={{ marginLeft: '6px', padding: '2px 7px', borderRadius: '999px', fontSize: '9px', fontWeight: '700', cursor: 'pointer', border: '1px solid', verticalAlign: 'middle', ...(allUpdate ? { background: '#fef2f2', color: '#dc2626', borderColor: '#fecaca' } : { background: '#f0fdf4', color: '#15803d', borderColor: '#86efac' }) }}
                >
                  {allUpdate ? '✕ Skip all' : '✓ Select all'}
                </button>
              )}
            </th>
          </tr>
...
function CrewTable({ rows, locations, onToggleAction, onEdit, onSelectAll }) {
  const dupRows = rows.filter(r => r.existingId)
  const allUpdate = dupRows.length > 0 && dupRows.every(r => r.action === 'update')
  return (
    <div style={{ overflowX: 'auto' }}>
              ...
              <td style={{ padding: '6px 10px' }}>
                {row.hotelNotFound ? (
                  ...
                  <span style={{ fontSize: '12px', color: '#374151' }}>
                    {row.hotel_id
                      ? (locations.find(l => l.uuid === row.hotel_id)?.name || row.hotel || row.hotel_id)
                      : (row.hotel || '–')}
                  </span>
                )}
              </td>
...
function AccommodationTable({ rows, locations, onToggleAction, onEdit, onSelectAll }) {
  const dupRows = rows.filter(r => r.existingId)
  const allUpdate = dupRows.length > 0 && dupRows.every(r => r.action === 'update')
  return (
    <div style={{ overflowX: 'auto' }}>
            ...
            <tr key={row._idx} style={{ background: rowBg(row, 'accommodation'), borderBottom: '1px solid #f1f5f9' }}>
              ...
              <td style={{ padding: '6px 10px' }}>
                {row.hotelNotFound ? (
                  ...
                  <span style={{ fontSize: '12px', color: '#374151' }}>
                    {row.hotel_id
                      ? (locations.find(l => l.uuid === row.hotel_id)?.name || row.hotel_name || row.hotel_id)
                      : (row.hotel_name || '–')}
                  </span>
                )}
              </td>
              ...
              <tr key={row._idx} style={{ background: bg, borderBottom: '1px solid #f1f5f9' }}>
                ...
                <td style={{ padding: '6px 10px', fontSize: '11px' }}>
                  {row.hotelNotFound
                    ...
                    : row.hotel_id
                      ? (locations.find(l => l.uuid === row.hotel_id)?.name || row.hotel_raw || '–')
                      : (row.hotel_raw || '–')}
                </td>
                <td style={{ padding: '6px 10px', textAlign: 'center' }}>
...
export function ImportModal({ open, mode: initialMode, productionId, locations = [], onClose, onImported, initialPhase, initialRows, initialNewHotels, initialDetectedMode, initialSelMode, initialFile }) {
  ...
  async function handleDriveSync(fileId) {
    ...
    try {
      ...
      await loadDriveFiles()
      // Imposta l'errore DOPO loadDriveFiles, altrimenti loadDriveFiles lo cancella con setDriveError(null)
  ...
  function selectAllRows(action) {
    setRows(prev => prev.map(r => r.existingId ? { ...r, action } : r))
  }
  ...
  function renderPreviewTable() {
        ...
        <>
          {crewRows.length > 0 && (
            <div style={{ marginBottom: '20px' }}>
              ...
              <CrewTable rows={crewRows} locations={locations} onToggleAction={toggleRowAction} onEdit={editRowField} onSelectAll={selectAllRows} />
            </div>
          )}
          {fleetRows.length > 0 && (
            <div>
              ...
              <FleetTable rows={fleetRows} onToggleAction={toggleRowAction} onEdit={editRowField} onSelectAll={selectAllRows} />
            </div>
          )}
        </>
      )
    ...
    if (effectiveDisplayMode === 'fleet') {
      return <FleetTable rows={goodRows} onToggleAction={toggleRowAction} onEdit={editRowField} onSelectAll={selectAllRows} />
    }
    if (effectiveDisplayMode === 'crew') {
      return <CrewTable rows={goodRows} locations={locations} onToggleAction={toggleRowAction} onEdit={editRowField} onSelectAll={selectAllRows} />
    }
    if (effectiveDisplayMode === 'accommodation') {
      return <AccommodationTable rows={goodRows} locations={locations} onToggleAction={toggleRowAction} onEdit={editRowField} onSelectAll={selectAllRows} />
    }
