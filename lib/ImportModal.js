                  ...
                  {newHotels.length > 0 && (
                    <div style={{ marginBottom: '16px', border: '1px solid #bae6fd', borderRadius: '10px', overflow: 'hidden' }}>
                      <div style={{ padding: '12px 16px', background: '#e0f2fe' }}>
                        ...
                        <div style={{ fontSize: '11px', color: '#0369a1', marginTop: '2px' }}>Hotels not found in your locations — will be added before importing crew</div>
                      </div>
                      <div>
                        {newHotels.map((h, i) => (
                          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '8px 16px', borderBottom: i < newHotels.length - 1 ? '1px solid #e0f2fe' : 'none', background: h.action === 'skip' ? '#f8fafc' : 'white' }}>
                            <span style={{ flex: 1, fontSize: '13px', fontWeight: '600', color: h.action === 'skip' ? '#94a3b8' : '#374151' }}>🏨 {h.name}</span>
                            <button
                              onClick={() => setNewHotels(prev => prev.map((x, xi) => xi === i ? { ...x, action: x.action === 'skip' ? 'add' : 'skip' } : x))}
                              style={{ padding: '4px 10px', borderRadius: '6px', fontSize: '10px', fontWeight: '700', cursor: 'pointer', border: '1px solid', whiteSpace: 'nowrap', flexShrink: 0, ...(h.action === 'skip' ? { background: '#f0fdf4', color: '#15803d', borderColor: '#86efac' } : { background: '#e0f2fe', color: '#0369a1', borderColor: '#bae6fd' }) }}>
                              {h.action === 'skip' ? '+ Add' : 'Skip'}
                            </button>
                          </div>
                        ))}
                      </div>
