...
function rowBg(row, mode) {
  if (mode === 'accommodation') {
    ...
    if (!row.existingId)                          return '#fef2f2'   // 🔴 rosso — non trovato in DB
        ...
        <tbody>
          {rows.map((row) => (
            <tr key={row._idx} style={{ background: rowBg(row, 'accommodation'), borderBottom: '1px solid #f1f5f9' }}>
              ...
              <td style={{ padding: '6px 10px', textAlign: 'center', whiteSpace: 'nowrap' }}>
                {!row.existingId ? (
                  <span style={{ padding: '3px 8px', borderRadius: '999px', fontSize: '10px', fontWeight: '700', background: '#fef2f2', color: '#dc2626', border: '1px solid #fecaca' }}>
                    ❌ Not found
                  </span>
                ) : (
                  <button
