...
export function ImportModal({ open, mode: initialMode, productionId, locations = [], onClose, onImported }) {
  ...
  async function parseFile(file) {
    ...
    try {
      ...
      // Aggiungi _idx e pre-fill defaults capacity/pax_suggested per tipo
      const enriched = (data.rows || []).map((r, i) => {
        ...
        return {
          ...r,
          _idx: i,
          // Pre-fill solo se null — l'utente vede i valori default e può correggerli
            ...
            <div style={{ padding: '24px' }}>
              ...
              <div style={{ marginBottom: '20px' }}>
                ...
                <textarea
                  ...
                  style={{ marginTop: '10px', width: '100%', minHeight: selMode === 'custom' ? '80px' : '54px', padding: '10px', border: '1px solid #e2e8f0', borderRadius: '8px', fontSize: '13px', resize: 'vertical', boxSizing: 'border-box', color: '#0f172a', background: selMode === 'custom' ? 'white' : '#f8fafc' }}
                />
              </div>
