...
export async function searchCrewForContacts(productionId, query, excludeCrewIds = []) {
  ...
  let req = supabase
    .from('crew')
    .select('uuid, display_id, full_name, role, department, phone, email')
    .eq('production_id', productionId)
    .ilike('full_name', `%${q}%`)
  ...
  if (syncWithCrew) {
    ...
    if (crewIds.length > 0) {
      const { data: crew } = await supabase
        .from('crew')
        .select('uuid, full_name, role, department, phone, email')
        .in('uuid', crewIds)
      for (const c of (crew || [])) crewById[c.uuid] = c
    }
  }
...
export async function addCrewContact(productionId, crew, opts = {}) {
  if (!productionId || !crew?.uuid) throw new Error('productionId and crew object required')
