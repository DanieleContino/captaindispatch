    ...
    if (m.direction === 'OUT') {
      ...
      if (gapDays > 0) {
        addWarning(m.crew_id, 'OUT_AFTER_CHECKOUT',
          `Volo OUT il ${fmtEU(m.travel_date)} — hotel check-out ${fmtEU(relevantStay.departure_date)}. Gap di ${gapDays} nott${gapDays === 1 ? 'e' : 'i'}: verifica date volo o hotel.`
        )
      }
    }
  }
    ...
    if (!hasOUT) {
      ...
      if (stayWithDep) {
        addWarning(crewUuid, 'MISSING_RETURN_FLIGHT',
          `Nessun rientro registrato — check-out hotel il ${fmtEU(stayWithDep.departure_date)}. Aggiungi volo/treno/OA di rientro o verifica la data di check-out.`
        )
      }
    }
    ...
    if (!hasIN) {
      ...
      if (stayWithArr) {
        addWarning(crewUuid, 'MISSING_ARRIVAL_FLIGHT',
          `Nessun arrivo registrato — check-in hotel il ${fmtEU(stayWithArr.arrival_date)}. Aggiungi volo/treno/OA di arrivo o verifica la data di check-in.`
        )
      }
    }
  }
